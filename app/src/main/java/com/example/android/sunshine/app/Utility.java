...
public class Utility {
    public static void  updateWatchFace(GoogleApiClient mGoogleApiClient, Context context ){
        ...
        Cursor data = context.getContentResolver().query(weatherForLocationUri,null,null,null,sortOrder );
        if (data != null) {
            data.moveToFirst();
        }
        int  weatherId = 0;
        ...
        double Tmin= 0;
        if (data != null) {
            weatherId = data.getInt(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID));
            Tmax = data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));
            Tmin= data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
        }
        if (data != null) {
            data.close();
        }
        PutDataMapRequest dataMap = PutDataMapRequest.create ("/weather");
        dataMap.getDataMap().putString("key", String.valueOf(weatherId) + "`" +
                Utility.formatTemperature(context, Tmax)  + "`" +Utility.formatTemperature(context, Tmin)  );
